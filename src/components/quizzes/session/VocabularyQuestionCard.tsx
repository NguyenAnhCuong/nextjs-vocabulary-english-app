"use client";
// src/components/quiz/session/VocabularyQuestionCard.tsx

import { Box, Typography, Stack, Paper } from "@mui/material";
import CheckCircleIcon from "@mui/icons-material/CheckCircle";
import CancelIcon from "@mui/icons-material/Cancel";
import type { VocabularyQuestion } from "@/types/quiz";
...
export default function VocabularyQuestionCard({ question, globalIndex, chosen, onAnswer, submitted }: Props) {
  return (
    <Paper
      elevation={0}
      sx={{
      ...
      <Stack spacing={1}>
        {question.options.map((opt, oi) => {
          ...
          return (
            ...
            >
              <Box sx={{
                minWidth: 26, height: 26, borderRadius: 1.5,
                bgcolor: isChosen || (submitted && isCorrect) ? borderColor : "divider",
                color: "#fff",
              ...
              <Typography variant="body2" sx={{ color, fontWeight: isChosen ? 700 : 400, flex: 1 }}>
                {opt}
              </Typography>
              {submitted && isCorrect && <CheckCircleIcon sx={{ fontSize: 18, color: "success.main" }} />}
              {submitted && isChosen && !isCorrect && <CancelIcon sx={{ fontSize: 18, color: "error.main" }} />}
            </Box>
          );
        })}
      </Stack>
